"""This module wraps the specifications of the training process for Dynamical networks to produce "good" samples.
NOTE: NOT IMPLEMENTED - SEEMS I DON'T NEED THIS 

1. use KL_divergence loss to place final locations to equally partition the rotation space
2. use regularisation_loss to ensure small outputs
...
class DnTrainer:
    def __init__(self, config=None):
        ...
        self.regularisation_loss = self._regularisation_loss_functions(config["regularisation_loss"])
        ...
        self.loss_fn = ELBO(self.regularisation_loss, self.kl_divergence_loss, config.get("annealing", False), config.get("annealing_function", None))
    ...
    def train(
        ...
        for e in range(epochs):
            ...
            for i, (data, labels) in enumerate(train_loader):
                ...
                self.writer.add_scalar("training regularisation loss", self.loss_fn.current_regularisation_loss, i + e * len(train_loader))
    ...
    def validate(self, val_loader, epoch, epochs = 0, early_stopping = False):
        ...
        with torch.no_grad():
            for i, (data, labels) in enumerate(val_loader):
                ...
                val_regularisation_loss += self.loss_fn.current_regularisation_loss
                val_kl_loss += self.loss_fn.current_kl_divergence_loss

            avg_val_loss = val_total_loss / len(val_loader)
            avg_val_regularisation_loss = val_regularisation_loss / len(val_loader)
            ...
            # Log additional loss components
            self.writer.add_scalar("validation regularisation loss", avg_val_regularisation_loss, epoch)
...
class CombinedLoss(nn.Module):
    def __init__(self, regularisation_loss, kl_divergence_loss, annealing = False, annealing_function = None):
        super(ELBO, self).__init__()
        self.regularisation_loss = regularisation_loss
        self.kl_divergence_loss = kl_divergence_loss
        self.current_regularisation_loss = 0
    ...
    def forward(self, x, x_hat, mu, log_var, e = None, E = None):
        self.current_regularisation_loss = self.regularisation_loss(x)
        self.current_kl_divergence_loss = self.kl_divergence_loss(mu, log_var)
        # print(mu.shape)
        # print(log_var.shape)
        if self.annealing:
            return self.current_regularisation_loss + self.annealing_function(e, E) * self.current_kl_divergence_loss
        else:
            return self.current_kl_divergence_loss + self.current_regularisation_loss
